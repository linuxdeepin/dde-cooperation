...
public:
...
#else
            if (0 == mprotect(pageof(pstub->fn), m_pagesize * 2, PROT_READ | PROT_WRITE))
#endif
            {
    ...
    void set(T addr, S addr_stub)
    {
        ...
        auto pstub = std::make_unique<func_stub>();
        //start
        pstub->fn = fn;
        ...
        if(0 == VirtualProtect(pageof(pstub->fn), m_pagesize * 2, PAGE_EXECUTE_READWRITE, &lpflOldProtect))
#else
        if (-1 == mprotect(pageof(pstub->fn), m_pagesize * 2, PROT_READ | PROT_WRITE))
#endif
        {
            throw std::runtime_error("stub set memory protect to w+r+x failed");
        }
        ...
        m_result.insert(std::pair<char*,func_stub*>(fn, pstub.release()));
        return;
    }
...
#endif
        {
            throw std::runtime_error("stub reset memory protect to w+r+x failed");
        }
...
private:
    ...
    bool distanceof(char* addr, char* addr_stub)
    {
        ...
        if((sizeof(addr) > 4) && (diff > 0x7FFFFFFF || diff < -0x7FFFFFFF - 1))
        {
            return true;
        }
        return false;
